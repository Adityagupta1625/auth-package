...
const connectionString = 'rediss://red-clmlgokjtl8s73a5t110:SWtnFi6BIMjvBw6Ca4SGFOwKF4ieHcKE@oregon-redis.render.com:6379'
...
describe('insertOTP', () => {
  test('should insert OTP into Redis', async () => {
    ...
    // Assert
    expect(consoleSpy).toHaveBeenCalledWith('OTP inserted into Redis successfully.')
...
describe('verifyOTP', () => {
  ...
  test('should return false if OTP is invalid', async () => {
    ...
    // Act
    const result = await verifyOTP({email: data.email,otp:"123789"}, connectionString)
