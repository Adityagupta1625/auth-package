 ...
 */
export const insertOTP = async (data: dbTypes.dbData, connectionString: string): Promise<void> => {
  try {
 ...
 */
export const verifyOTP = async (data: dbTypes.dbData, connectionString: string): Promise<boolean> => {
  try {
    // Get a Redis client by connecting to the Redis server.
    const client = await redisClient(connectionString);
    ...
    if (storedOTP === data.otp) {
      ...
      await client.del(data.email) // Delete the OTP from Redis.
      await client.disconnect() // Disconnect from the Redis server.
      return true;
    } else {
